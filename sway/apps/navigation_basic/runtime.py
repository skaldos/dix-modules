...
class Runtime:
    def __init__(self,*,context:ApplicationRuntimeContext,config:Mapping[str,object],ipc:Api)->None: self.context,self.config,self.ipc=context,config,ipc
    ...
    def _move(self,direction:str)->dict[str,object]:
        ...
        if result is not None: raise TypeError("focus_direction must return None")
        focused=_id(self.ipc.require("focused_con_id")())
        return {"direction":direction,"origin_id":origin,"focused_id":focused,"changed":focused!=origin}
