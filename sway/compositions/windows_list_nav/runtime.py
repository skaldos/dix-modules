...
class Runtime:
    def __init__(
        ...
        *,
        ...
        self.context, self.config, self.basic_nav, self.ipc = (
            context,
            config,
            basic_nav,
            ipc,
    ...
    def left(self, window_ids: list[int] | None = None) -> dict[str, object]:
        return self._move("left", [] if window_ids is None else window_ids)

    def right(self, window_ids: list[int] | None = None) -> dict[str, object]:
        return self._move("right", [] if window_ids is None else window_ids)

    def up(self, window_ids: list[int] | None = None) -> dict[str, object]:
        return self._move("up", [] if window_ids is None else window_ids)

    def down(self, window_ids: list[int] | None = None) -> dict[str, object]:
        return self._move("down", [] if window_ids is None else window_ids)

    def _move(self, direction: str, window_ids: list[int]) -> dict[str, object]:
        members = _ids(window_ids, "window_ids")
        origin = _id(self.ipc.require("focused_con_id")(), "focused con_id")
        if not members:
            return _result(direction, origin, origin, False, True, [origin], [])
        ...
        for _ in range(max(1, len(live_ids) + 1)):
            previous = current
            step = _mapping(self.basic_nav.require(direction)(), "basic navigation")
